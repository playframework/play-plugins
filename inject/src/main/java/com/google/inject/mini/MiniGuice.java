...
public final class MiniGuice {
   ...
   */
  public static <T> T inject(Class<T> type, boolean fallbackToDefaultConstructor,  Object... modules) {
    ...
    miniGuice.addJitBindings(fallbackToDefaultConstructor);
  ...
  private void addJitBindings(boolean fallbackToDefaultConstructor) {
    ...
    while ((requiredKey = requiredKeys.poll()) != null) {
      ...
      addJitBinding(key, requiredKey.requiredBy, fallbackToDefaultConstructor);
    }
  }

  private void addJitBinding(Key key, Object requiredBy, boolean fallbackToDefaultConstructor) {
               ...
               try {
                  ...
                  for (Constructor c: constructors) {
                    if (c.getTypeParameters().length == 0) { 
                      injectedConstructor = c;
                      injectedConstructor.setAccessible(true);
                      break;
                    }
                  }
                  if (injectedConstructor == null) throw new NoSuchMethodException();
                  System.out.println("YAY");
                } catch (NoSuchMethodException ex) {
                    throw new IllegalArgumentException("Could not find default constructor "
                        + type + " required by " + requiredBy +" available constructors:"+type.getConstructors().length );
                }
            } else
            throw new IllegalArgumentException("No injectable constructor on "
                + type + " required by " + requiredBy);
