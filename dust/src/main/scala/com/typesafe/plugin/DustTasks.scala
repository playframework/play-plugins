...
trait DustTasks extends DustKeys {
  ...
  def compile(name: String, source: String): Either[(String, Int, Int), String] = {
    ...
    ctx.evaluateReader(
      scope,
      new InputStreamReader(this.getClass.getClassLoader.getResource("dust-full-mod-0.3.0.js").openConnection().getInputStream()),
      "dust.js",
      1, null)
      ...
      if (previousInfo != currentInfos) {
        ...
        val generated = (files x relativeTo(assetsDir)).flatMap {
          case (sourceFile, name) => {
            val msg = compile(sourceFile.getPath.replace(assetsDir.getPath , "").drop(1).replace("\\","/"), IO.read(sourceFile)).left.map {
